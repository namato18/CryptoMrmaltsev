...
if(shortBacktestInterval == '4hour' | shortBacktestInterval == '8hour'| shortBacktestInterval == '1hour'| shortBacktestInterval == '15min' |
  ...
  df1 = riingo_crypto_prices(shortBacktestCoins, start_date = Sys.Date() - as.numeric(shortBacktestTimeframe), end_date = Sys.Date(), resample_frequency = shortBacktestInterval, exchanges = "binance")
  df1 = df1[-nrow(df1),]
  df2 = riingo_crypto_latest(shortBacktestCoins, resample_frequency = shortBacktestInterval, exchanges = "binance")
  df = rbind(df.usd,df1,df2)
  df = df[-duplicated(df$date),]
}else{
  df = riingo_crypto_prices(shortBacktestCoins, start_date = Sys.Date() - as.numeric(shortBacktestTimeframe), end_date = Sys.Date(), resample_frequency = shortBacktestInterval, exchanges = "binance")
}
...
for(z in 1:length(shortBacktestTarget)){
  for(y in 1:length(confidenceBacktestAutomation)){
    for(n in 1:length(shortBacktestTP)){
      ...
      for(i in 1:length(shortBacktestCoins)){
        ...
        bst2 = s3read_using(FUN = readRDS, bucket = paste0("cryptomlbucket/TiingoBoosts"),
                            object = paste0("bst_",shortBacktestCoins[i],"_",shortBacktestInterval,"-",shortBacktestTarget[z],".rds"))
        
        
        ############################################# 
        ############################################# PREDICT CURRENT CANDLE
        predict.next1 = predict(bst, df)
        predict.next2 = predict(bst2, df)
        
        
        
        
        if(i == 1){
          predictions.comb = data.frame(predict.next1,
                                        predict.next2)
          predictions.comb$decision = 'LONG'
          predictions.comb$decision[predictions.comb$predict.next2 > predictions.comb$predict.next1] = 'SHORT'
          predictions.comb$predict.next = NA
          predictions.comb$predict.next[predictions.comb$predict.next1 >= predictions.comb$predict.next2] = 
            predictions.comb$predict.next1[predictions.comb$predict.next1 >= predictions.comb$predict.next2]
          predictions.comb$predict.next[predictions.comb$predict.next1 < predictions.comb$predict.next2] = 
            predictions.comb$predict.next2[predictions.comb$predict.next1 < predictions.comb$predict.next2]
          long.short = predictions.comb[,3]
          predictions.comb = predictions.comb[,c(4)]
          predictions.comb = data.frame(predictions.comb)
      ...
      if(length(which(is.na(woulda.bought))) != 0){
        long.short = long.short[which(!is.na(woulda.bought))]
      }
      ...
      df.purchases$LongShort = long.short
      ...
      df.purchases$PL[df.purchases$OH >= df.purchases$TakeProfit & df.purchases$LongShort == "LONG"] = 
        df.purchases$TakeProfit[df.purchases$OH >= df.purchases$TakeProfit & df.purchases$LongShort == "LONG"]
      df.purchases$PL[df.purchases$OL <= (df.purchases$TakeProfit *-1) & df.purchases$LongShort == "SHORT"] = 
        df.purchases$TakeProfit[df.purchases$OL <= (df.purchases$TakeProfit *-1) & df.purchases$LongShort == "SHORT"]
      
      df.purchases$PL[df.purchases$OH < df.purchases$TakeProfit & df.purchases$LongShort == "LONG"] =
        df.purchases$OC[df.purchases$OH < df.purchases$TakeProfit & df.purchases$LongShort == "LONG"]
      df.purchases$PL[df.purchases$OL > (df.purchases$TakeProfit * -1) & df.purchases$LongShort == "SHORT"] = 
        df.purchases$OC[df.purchases$OL > (df.purchases$TakeProfit * -1) & df.purchases$LongShort == "SHORT"] * -1
      ...
      colnames(df.purchases) = c("Open", "High", "Low", "Close", "Coin", "Time (UTC)", "Long/Short","Confidence Scores", "Open/High","Open/Low", "Open/Close", "Take Profit","Stop Loss", "PL")
  ...
  print(paste0(z, " out of: ", length(shortBacktestTarget)))
}

toc()

write.csv(df.examine, file = "df.examine.reefusdt.4hour.360day.longshort.csv")
